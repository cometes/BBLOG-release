...
- **스토리 뷰어**: 이벤트와 메인 스토리 챕터를 골라 앱 안에서 바로 읽을 수 있습니다.
  - 한국어 모드에서는 한국 서버에 정식 출시된 스토리의 공식 한국어 번역을 우선 사용합니다.
  - 영어 모드에서는 북미 서버에 정식 출시된 스토리의 공식 영어 번역을 우선 사용합니다.
  - 선택한 언어로 아직 정식 출시되지 않은 스토리는 Gemini AI로 번역할 수 있습니다.
- **번역 직접 수정**: Gemini 번역이 어색하면 뷰어에서 번역 대사나 이름표를 길게 눌러 직접 수정할 수 있습니다. 같은 화자의 이름표는 해당 챕터 전체에 함께 적용됩니다.
- **게임 화면 위 실시간 오버레이**: 일본 서버에서 직접 플레이하면서 게임 화면 위에 번역 자막을 띄울 수 있습니다. 대사창을 자동으로 인식해 다음 줄로 진행합니다.
- **커뮤니티 번역 공유**: 다른 사용자가 공유한 번역을 가져와 바로 읽거나, 자신의 번역을 공유할 수 있습니다.
...
- AI 번역에는 본인의 Google Gemini API 키가 필요합니다. 키는 튜토리얼 또는 설정에서 등록할 수 있으며, 사용량에 따라 비용이 발생할 수 있습니다.
...
- **Story viewer**: Choose an event or Main Story chapter and read it directly in the app.
  ...
  - Stories not yet officially released in the selected language can be translated with Gemini AI.
- **Edit Gemini translations**: Long-press a translated dialogue line or speaker nameplate to correct it. Renaming a speaker updates matching nameplates throughout the chapter.
...
- **Community translations**: Download translations shared by other users or share your own translations.
...
- AI translation requires your own Google Gemini API key. You can register it during the tutorial or later in Settings. Charges may apply depending on usage.
